...
import {clearError, setRegisterSuccess} from "../../state/auth/authSlice.js";
...
const RegisterPage = () => {
    ...
    const registerSuccess = useSelector((s) => s.auth.registerSuccess);
    ...
    useEffect(() => {
      ...
      }, [dispatch, clearError, setRegisterSuccess]);

    // 2. Lắng nghe: Nếu đăng ký thành công -> Chuyển sang login
    useEffect(() => {
        if (registerSuccess) {
            alert("Đăng ký thành công! Hãy đăng nhập.");
            dispatch(setRegisterSuccess(false)); // Reset trạng thái
            navigate("/login");
        }
